...
import { summarizeSession } from "../lib/sessionHistory.js";
...
export default function LogScreen({ onArchiveSession, onUndoArchiveSession }) {
  ...
  const resetDay = useCallback(() => {
    ...
    const summary = summarizeSession(day, previousDayState, dateISO, dayId);
    const archived = summary.doneSets > 0;
    if (archived) onArchiveSession(summary);
    ...
    showToast({
      ...
      duration: 30000,
      onAction: () => {
        setAllState((prev) => ({ ...prev, [dayId]: previousDayState }));
        if (archived) onUndoArchiveSession(summary.id);
      },
    });
  }, [allState, day, dayId, dateISO, onArchiveSession, onUndoArchiveSession, showToast]);
  ...
  return (
    <div
      style={{
        paddingBottom: timer !== null ? 220 : TAB_BAR_CLEARANCE,
      }}
    >
      <div
        style={{
